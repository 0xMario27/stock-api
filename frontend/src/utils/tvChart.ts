...
function ktime(k: Kline): Time {
  if (k.timestamp) return k.timestamp as Time;
  return k.date as Time;
}
...
export class TVChartManager {
  ...
  private rebuild(): void {
    ...
    candle.setData(kl.map(k => ({ time: ktime(k), open: k.open, high: k.high, low: k.low, close: k.close })));
    ...
    if (ind.vol) {
      ...
      vs.setData(kl.map(k => ({ time: ktime(k), value: k.volume??0, color: k.close>=k.open ? c.up+"50" : c.down+"50" })));
  ...
  private onCrosshair(param: any): void {
    ...
    const idx = this.klines.findIndex(k => ktime(k) === param.time);
